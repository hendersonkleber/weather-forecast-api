...
public interface WeatherForecastHourRepository extends JpaRepository<WeatherForecastHour, Long> {
    ...
    @Query("SELECT w FROM WeatherForecastHour w WHERE w.weatherForecastDay.id = :weatherForecastDayId AND w.hour = :hour")
    Optional<WeatherForecastHour> findByWeatherForecastDayIdAndHour(Long weatherForecastDayId, LocalTime hour);
